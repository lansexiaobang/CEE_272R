...
function [dcopf]=DCOPF(Y,PGl,PGu,PD,thetaL,thetaU,CQ,CL,CC,PF,slack)
    n=length(Y);
    B = imag(Y);
    x=1;        % Counter
...
cvx_begin quiet
    ...
    minimize(sum(PG.*PG.*CQ + PG.*CL+CC)) % objective function
    subject to
    ...
    for i=1:n           
        for j=(i+1):n
            count = count + 1;
            -PF(count) <= B(i,j)*(theta(i)-theta(j)) <= PF(count);
        end
    end
    
    %%%%%%%%%Bus phase angle constraints
    thetaL <= theta(n) <= thetaU;
